...
class ReportOfAccountantLastMonth extends Controller implements FromCollection, WithHeadings
{
    use Exportable;
    ...
    public function collection()
    {
        ...
        $orders = $dborder->whereBetween('created_at',[Carbon::now()->startOfMonth()->subMonth(1),$end])->where('id_status',5)->orderBy('id','DESC')->get();
        if($orders->isEmpty() == false){
            ...
            echo "<script>alert('Báo cáo doanh thu tháng trước rỗng !');window.location.href='".URL::previous()."'</script>";
        }    
    }
    ...
    public function export(){
        return Excel::download(new ReportOfAccountantLastMonth(), 'orders.xlsx');
   }
}
